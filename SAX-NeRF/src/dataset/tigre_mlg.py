...
class TIGREDataset_MLG(Dataset):
    ...
    TIGRE dataset.
    """
    def __init__(self, path, n_rays=1024, type="train", window_size = [32, 32], window_num = 4, device="cuda", biased_sampling=False):
        super().__init__()
    ...
    def __getitem__(self, index):
        if self.type == "train":
            ...
            # 选取 window_inds
            if getattr(self, 'biased_sampling', False):
                # 偏置采样：按 window 内投影均值加权选择，优先选核桃区域
                window_means = projs_window.mean(dim=-1).mean(dim=-1).cpu().numpy()
